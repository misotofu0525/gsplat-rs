...
`offscreen_geometry_pipeline=resident_sorted_indices`. Benchmark the same path with:
...
mobile, with per-camera-frame CPU sorting and resident sorted indices as its
default policy:
...
Windowed and offscreen rendering use the same resident shader/resource layout.
