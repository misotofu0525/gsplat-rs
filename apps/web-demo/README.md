...
```text
http://127.0.0.1:4173/apps/web-demo/
```

Do not open `apps/web-demo/index.html` with `file://`. Browser security rules
block the wasm package and root-relative dataset fetches in that mode.

Open the larger flower smoke scene directly with:

```text
http://127.0.0.1:4173/apps/web-demo/?dataset=flowers
```
...
```text
http://127.0.0.1:4173/apps/web-demo/?gsplat_benchmark=true&gsplat_benchmark_frames=120&gsplat_benchmark_warmup_frames=10&gsplat_benchmark_yaw_step=0.001&gsplat_surface_sort_interval=2
```
