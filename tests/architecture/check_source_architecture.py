...
def grouped_crate_paths(code: str) -> Iterable[tuple[list[str], int]]:
    """Yield top-level alternatives from `crate::{a, b::c}` use groups."""

    for match in re.finditer(r"\bcrate\s*::\s*\{(?P<body>[^;]*)\}\s*;", code, re.DOTALL):
        ...
        for index, char in enumerate(body + ","):
            if char == "{":
                ...
                item_match = re.match(r"(?:r#)?([A-Za-z_]\w*)", item)
                if item_match:
                    yield [item_match.group(1)], match.start("body") + start
                start = index + 1
...
def closed_tasks(progress: str) -> set[str]:
    closed: set[str] = set()
    heading_re = re.compile(r"^###\s+([A-Za-z][A-Za-z0-9.-]*)\b")
    ...
    for line in progress.splitlines():
        ...
        if heading:
            ...
            continue
        state = re.match(r"^-\s+(?:Final state|State):\s+([A-Za-z]+)\s*$", line)
        if current and state and state.group(1).lower() in TERMINAL_STATES:
            closed.add(current)
        row = re.match(
            r"^\|\s*([A-Za-z][A-Za-z0-9.-]*)\s*\|\s*(Accepted|Rejected|Deferred)\s*\|",
            line,
            re.IGNORECASE,
        )
        if row:
            closed.add(row.group(1))
    return closed
...
def check_sizes(
    ...
    for entry in policy.get("grandfather", []):
        ...
        for field in ("baseline_physical_loc", "owner_task", "exit_condition"):
                ...
                if loc > baseline and not exception_active:
                    issues.append(
                        Issue(
                            ...
                            path,
                            f"{loc} physical LOC grew above A0 baseline {baseline}; grandfathered files are shrink-only",
                        )
                    )
                elif loc < baseline:
                    issues.append(
                        Issue(
                            "notice",
                            "size.grandfather_shrink",
                            path,
                            f"shrunk from A0 baseline {baseline} to {loc}; lower the checked baseline in the extracting task",
                        )
                    )
            else:
...
def check_dependencies(
    ...
    for path in rust_paths:
        ...
        if path_matches(path, gpu_paths):
            ...
            resolved.extend(grouped_crate_paths(code))
            emitted: set[str] = set()
            ...
            for name, field_type, offset in struct_fields(code):
                ...
                categories: list[tuple[str, bool, str]] = [
                    (
                        "plan_selection",
                        bool(
                            re.search(r"(?:selected|active|current|requested)_plan(?:_id)?$", lowered)
                            or re.search(r"^plan_(?:selection|selector|controller|set)$", lowered)
                            or re.search(r"\b(?:WholePlanController|PlanSet)\b", field_type)
                        ),
                    ...
                    (
                        "adaptive_state",
                        "adaptive" in lowered
                        or bool(re.search(r"\b(?:AdaptiveState|WholePlanController|PlanSampler)\b", field_type)),
...
def check_orchestration(
    root: pathlib.Path, policy: dict[str, Any], completed: set[str]
) -> list[Issue]:
...
def check_repository(root: pathlib.Path, policy: dict[str, Any]) -> tuple[list[Issue], dict[str, int]]:
    ...
    progress_path = root / policy["progress_file"]
    progress = progress_path.read_text(encoding="utf-8") if progress_path.is_file() else ""
    completed = closed_tasks(progress)
    issues = check_sizes(root, policy, sources, completed)
    issues.extend(check_dependencies(root, policy, sources.get("rust", [])))
    issues.extend(check_orchestration(root, policy, completed))
