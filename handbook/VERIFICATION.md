...
- Expect `offscreen_geometry_pipeline=resident_sorted_indices` in the PNG and
...
- Use the interactive viewer when changing windowed presentation or camera
  interaction behavior. It uses the same `SurfaceRenderSession` and resident
...
observed values and includes the resident resource preflight report.
...
- Resident Surface path smoke (after wasm build):
...
- Expected wasm benchmark output includes `renderer=resident_sorted_indices`.
...
- This is the proof path for the shared Rust `wgpu` renderer and local Web SDK
  ...
  report `surface=wasm-wgpu`, `renderer=resident_sorted_indices` in benchmark output,
...
- Expected first frame includes `Kitsune shrine`, `LIVE`, a non-zero splat count,
  ...
  `state=rendering`. Expect
  `drawn=<surface_instances>/<visible_instances>`.
...
- If you touch renderer, sorting, or perf-sensitive code, run `cargo run --release -p bench-runner -- tests/datasets/minimal_ascii.ply 120 --warmup-iterations 10 --max-avg-gpu-complete-ms 250` and consider the long-stability script. The runner reports CPU preprocessing, CPU sort, encode/submit CPU wall, GPU wait, GPU-complete, nearest-rank frame distributions, missed-frame counts, and structured resident-resource preflight together with adapter/backend/driver metadata. Use the artifact route above when the result will be retained or compared. Surface/WASM output additionally reports render/submit and frame-wall phases; compatibility CPU-geometry fields stay zero on the resident path.
