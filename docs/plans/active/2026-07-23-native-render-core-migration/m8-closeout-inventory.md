...
| M8-D15 | After all source, documentation, policy, active-path and populated-report edits are committed, freeze one pre-archive candidate SHA. The current-SHA matrix was run on `c6ea13e41bde1a9d0f8af7252d343538603bc059`, but independent review rejected that tree because of the stale-status P1 repaired by this slice; those results are predecessor evidence rather than validation of the next candidate. Root integration of this repair defines the resulting new SHA, which must receive a fresh cutover clean-tree local matrix and independent finite review before D13. Use repository commands, not an ad-hoc substitute. | Root verification owner | [cutover verification catalog](cutover.md#verification-command-catalog) and verification handbook | Formatting, workspace check/tests, strict Clippy, Rustdoc, architecture self/real-tree checks, dependency policy, C/consumer checks affected by edits and committed-range diff checks; join every result to the post-repair candidate SHA | **Pending; freeze and verify the post-repair SHA** | Device/browser runs only where a new factual claim requires them. The `c6ea13e` run and historical endpoint artifacts retain their own SHAs; they are not post-repair candidate qualification. |
