...
| M8-D16 | Keep release distribution facts narrow: AAR/XCFramework/npm-compatible files are direct prerelease artifacts, not Maven, remote binary SwiftPM, npm or crates.io publication. M8e confirmed the `v0.1.3` GitHub prerelease assets and absent registries; M8f reconciles Apple docs without performing any release operation. | Release-boundary reviewer | `RELEASING.md`, ROADMAP, root README, release workflow, M8e read-only distribution audit | Cross-document terminology search; release artifact-name audit | **M8f repair candidate; independent review required** | Actual release and remote settings are **Deferred**. |
