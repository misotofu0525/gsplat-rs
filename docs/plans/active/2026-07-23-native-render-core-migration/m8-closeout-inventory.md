...
| M8-D14 | Resolve the two external ownership review records `IO-PLY-1` and `IO-SPZ-1`. Open independent visible IO ownership reviews and either remove each grandfather after a cohesive ownership exit, re-prove one responsibility and close the record, or introduce a separately reviewed cataloged owner/follow-up without renewing the current M8 review. M8 cannot become Accepted while `review_task: M8` remains unresolved. | Separate visible `IO-PLY-1` and `IO-SPZ-1` review owners; root integrates the decision | `tests/architecture/source_architecture_policy.json` `grandfather` entries and checker `grandfather.review_due` behavior | Architecture self-tests; real-tree policy with `M8 = Accepted` in a candidate ledger; focused IO tests if source changes are separately authorized | **Deferred at M8a; required before M8 acceptance** | No external platform evidence. No source edit is authorized by this inventory. |
...
| --- | --- | --- | --- | --- |
| Chrome/WebGPU at accepted M7 SHA | **Deferred** | M7 WASM compilation and historical M4 Chrome functional/quality evidence, labelled with their own SHAs | Compilation, WebGL2, or an older Chrome run cannot become a `2739f48` browser result | Future Web qualification task; exact locked `wasm-bindgen-cli 0.2.121`, real Chrome/WebGPU run and canonical artifact validators |
