...
| M8-D11 | Normalize the five hard-coded active-bundle paths before the directory move: `m7-r3-renderer-offscreen-host.md`, `m7-r4-renderer-facade.md`, `m7-standalone-packed-entry-closeout.md`, `m7-legacy-packed-presenter-graph-closeout.md`, and `m7-public-callsite-api-abi-ledger.md`. Commands intended to be rerunnable after archival now point to `completed`; the sole retained D11 `active` path is an immutable pre-archive `git diff --no-index` transcript explicitly labelled historical and not rerunnable after archival. The independently reviewed repair was integrated as `76188933a87a37f50175fa80720a429d50608ee7`. | M8 archive/link writer | Repository-wide search for `docs/plans/active/2026-07-23-native-render-core-migration` | Exact-path classification, current and simulated post-archive relative-link checks, path mapping, `git diff --check` and `git show --check` passed | **Accepted** | None. |
| M8-D12 | Produce one populated [final closeout report](m8-final-closeout.md) containing the final architecture/deprecation ledger, exact M0--M8 SHA ledger, verification results, accepted limitations and non-claims. The report was independently reviewed and integrated by root as `315f78e059ee0cd94b7653b4465e1937039d0131`; its follow-up status-order repair was integrated as `c6ea13e41bde1a9d0f8af7252d343538603bc059`. | M8 final-ledger writer; root accepts | M0 cutover contract, `progress.md`, milestone closeouts, Git object identities | Independent fixed-SHA review, SHA ancestry/tree checks, links and final diff hygiene passed for the D12 report chain | **Accepted** | Deferred endpoints stay explicit rather than blocking truthful closeout. |
| M8-D13 | Root moved this entire bundle atomically from `docs/plans/active/` to `docs/plans/completed/` in this pure archive/state commit and set `M8 = Accepted`. The architecture policy already names both locations and must select only the completed ledger after the move. | Root M8 integration owner | Cutover M8 archive requirement, `RELEASING.md`, `tests/architecture/source_architecture_policy.json` | Full relative-link checker, architecture self-tests and real-tree policy, stale active-path search, `git diff --summary` and clean-tree check run after the move; D15 is not rerun solely because paths moved | **Accepted; Package M complete** | None. |
...
| M8-D15 | Root froze post-repair candidate `63678fcdd5d26973ec21427071ea9cea20bc5b24`, reran the local clean-tree matrix on that exact SHA, and obtained a separate visible finite review. The former `c6ea13e` matrix remains rejected predecessor evidence and was not inherited. | Root verification owner | [cutover verification catalog](cutover.md#verification-command-catalog) and verification handbook | Formatting, workspace check/tests, strict Clippy, Rustdoc, architecture self/real-tree checks, dependency policy, Metal conformance, C/Swift/JNI smoke, WASM/package checks, Android APK/AAR and XCFramework builds, and committed-range diff checks passed on `63678fc` | **Accepted; final review P0/P1 = 0** | Chrome/WebGPU, A065, iOS Simulator, physical iPhone and Windows/Linux runtime stay Deferred unless an actual endpoint run is retained. |
