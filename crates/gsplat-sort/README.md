...
CPU radix sort for `gsplat-rs` render ordering.
...
`SortBackend` trait implemented by `CpuSortBackend`:
...
Experimental GPU ordering lives in `gsplat-render-wgpu`, not here.

## Position in the workspace

`gsplat-render-wgpu` drives CPU ordering per frame; this crate stays
