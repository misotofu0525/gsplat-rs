// Hardware SortedAlpha quad draw from the binary16-axis rank-indexed projection cache.

@group(0) @binding(0)
var<storage, read> projected_center_source: array<vec4<f32>>;
@group(0) @binding(1)
var<storage, read> projected_axes: array<vec2<u32>>;
@group(0) @binding(2)
...
// The fragment shader rejects alpha below 1/255. Restricting the quad to the
// slightly wider 1/256 iso-contour preserves every fragment that can survive,
// including the equality boundary, while avoiding guaranteed-discard work.
// The instance is still issued; only its mathematically empty outer support is
// removed. Scaling position and `local` together preserves the Gaussian map.
fn alpha_extent_scale(alpha: f32) -> f32 {
  return sqrt(clamp(log(max(alpha, 1e-12) * 256.0) / 4.5, 0.0, 1.0));
}

// Direct and compacted drawing deliberately retain this same operation tree,
// which also matches the established resident/global image oracle.
...
fn vs_main(
  ...
  let center_source = projected_center_source[instance_index];
  var out: VsOut;
  let packed_axes = projected_axes[instance_index];
  let axis_u = unpack2x16float(packed_axes.x);
  let axis_v = unpack2x16float(packed_axes.y);
