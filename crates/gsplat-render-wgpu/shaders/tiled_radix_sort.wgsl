...
fn item_index(
  ...
  return logical_workgroup_id(wg_id, num_wg) * ITEMS_PER_GROUP
    + round * WORKGROUP_SIZE + lane;
}
...
fn init_source_keys(
  ...
  for (var round = 0u; round < ITEMS_PER_THREAD; round += 1u) {
    ...
    if (index < tiled_params.entry_count) {
      // Ascending radix on bitwise-not yields descending source ID, which is
      // the reverse of the current BTF oracle's equal-depth source order.
      generated_keys[index] = ~entries[index].source_id;
...
fn rekey_depth(
  ...
  for (var round = 0u; round < ITEMS_PER_THREAD; round += 1u) {
    let index = item_index(lane, round, wg_id, num_wg);
    if (index < tiled_params.entry_count) {
      generated_keys[index] = entries[generated_ids[index]].depth_key;
    }
  }
}
...
fn rekey_tile(
  ...
  for (var round = 0u; round < ITEMS_PER_THREAD; round += 1u) {
    let index = item_index(lane, round, wg_id, num_wg);
    if (index < tiled_params.entry_count) {
      generated_keys[index] = entries[generated_ids[index]].tile_id;
    }
  }
}
...
fn scatter(
  ...
  for (var round = 0u; round < ITEMS_PER_THREAD; round += 1u) {
    ...
    if (valid) {
      ...
      let output_index = radix_prefix[slot * pass_params.group_count + group]
        + local_rank;
