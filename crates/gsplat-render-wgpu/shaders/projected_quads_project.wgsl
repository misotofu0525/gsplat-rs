// Exact rank-indexed projection cache for Resident SortedAlpha quads.
...
// Both output planes are exactly 16 bytes per visible rank. Keeping them in
// separate bindings preserves the portable 128 MiB / 8,388,608-splat limit.
@group(0) @binding(5)
var<storage, read_write> projected_center_source: array<vec4<f32>>;
@group(0) @binding(6)
var<storage, read_write> projected_axes: array<vec4<f32>>;
@group(0) @binding(7)
...
fn main(
  ...
  if (rank < visible_count) {
    ...
    projected_axes[rank] = vec4<f32>(projected.axis_u, projected.axis_v);
    ...
    projected_axes[rank] = vec4<f32>(0.0);
  }
