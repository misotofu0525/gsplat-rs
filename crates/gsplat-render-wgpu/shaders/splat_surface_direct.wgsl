...
fn eval_color(idx: u32, source: SurfaceSourceElem, alpha: f32) -> vec4<f32> {
  ...
  let sh_rgb = vec3<f32>(
    eval_sh_channel(idx, 0u, source.color_dc.x, dir, degree),
    eval_sh_channel(idx, 1u, source.color_dc.y, dir, degree),
    eval_sh_channel(idx, 2u, source.color_dc.z, dir, degree),
  );
...
fn vs_main(
  ...
  let offset = projected.axis_u * quad.x + projected.axis_v * quad.y;
