//! Canonical CPU reference raster-data preparation.
//!
//! This owner contains only the established CPU implementation for covariance
//! preparation, projection, SH evaluation, and draw-instance construction.
//! Ordering policy, scene transactions, Exact runtime state, and publication
...
use crate::data::{CameraCovarianceTerms, GpuInstance, ShColorLayout};
...
#[derive(Clone, Copy)]
pub(crate) struct InstanceBuildParams {
    ...
    lim_x: f32,
    lim_y: f32,
    pub(crate) blur_cov_x: f32,
    pub(crate) blur_cov_y: f32,
    pub(crate) view_rot: [[f32; 3]; 3],
}

impl InstanceBuildParams {
    pub(crate) fn new(camera: &Camera, config: RendererConfig) -> Option<Self> {
...
pub(crate) fn project_world_covariance_terms_to_ndc(
    p_cam: Vec3f,
...
pub(crate) unsafe fn sh_color_unchecked(
    scene: &SceneBuffers,
    index: usize,
    dir: [f32; 3],
...
mod tests {
    ...
    fn reference_scene() -> SceneBuffers {
        SceneBuffers {
            positions: vec![Vec3f::new(-0.25, 0.0, 2.0), Vec3f::new(0.25, 0.0, 2.0)],
            opacity: vec![0.0, 0.0],
            scale_xyz: vec![[-3.0; 3], [-3.0; 3]],
            rotation_xyzw: vec![[0.0, 0.0, 0.0, 1.0]; 2],
            color_dc: vec![[0.25, -4.0, 0.75], [0.5, 0.0, -4.0]],
            sh_degree: 0,
            sh_rest: None,
        }
    }
    ...
    #[test]
    fn instance_builder_preserves_requested_order_and_skips_invalid_ids() {
        let scene = reference_scene();
        let covariances = precompute_world_covariances(&scene);
        let alpha = precompute_alpha_values(&scene);
        ...
        assert!(instances[0].center_and_axis_u[0] > 0.0);
        assert!(instances[1].center_and_axis_u[0] < 0.0);
    }

    #[test]
    fn degree_zero_sh_keeps_reference_lower_only_clamp() {
        let scene = reference_scene();
        let layout = ShColorLayout::new(&scene);
        let rgb = unsafe { sh_color_unchecked(&scene, 0, [0.0, 0.0, 1.0], layout) };
        const C0: f32 = 0.282_094_8;

        assert_eq!(rgb[0], C0 * 0.25 + 0.5);
        assert_eq!(rgb[1], 0.0);
        assert_eq!(rgb[2], C0 * 0.75 + 0.5);
    }

    #[test]
    fn covariance_cache_matches_the_single_source_oracle_bit_for_bit() {
        let scene = reference_scene();
        let cached = precompute_world_covariances(&scene);

        for (index, covariance) in cached.iter().enumerate() {
            assert_eq!(
                *covariance,
