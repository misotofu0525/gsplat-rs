...
- Streamed SOG (`lod-meta.json` plus chunk directories) is metadata-first.
  ...
  independent source / decoded / gaussian budgets, and returns only that
  subset. Native builds decode missing chunks in parallel. It never loads
  every chunk into one hidden full scene.
